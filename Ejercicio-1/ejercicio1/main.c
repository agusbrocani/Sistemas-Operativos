...
#define N 3 /// cantidad de hijos que vamos a crear, en este caso elegí 3

int main() {

    /// creo memoria compartida
    int id_memoria = crear_memoria_compartida(); // zona memoria compartida
    DatosCompartidos* datos = conectar_memoria(id_memoria); // la vincula a nuestro proceso y devuelve un puntero que usamos como si fuera una variable normal.
    ...
    /// creo el semáforo
    iniciar_semaforo(); // creo un semáforo con valor 1. lo uso para que solo un proceso hijo a la vez pueda modificar el vector.
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (pid == 0) { // estamos dentro del hijo, así que llamamos a tarea_hijo() con su número e inmediatamente empieza su trabajo.
            tarea_hijo(i, datos);
    ...
    for (int i = 0; i < N; i++) waitpid(hijos[i], NULL, 0);
